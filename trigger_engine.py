...
def get_weekly_risk(lat, lon):
    ...
    try:
        ...
        w_json = requests.get(w_url, timeout=10).json()
        ...
        aqi_json = requests.get(aqi_url, timeout=10).json()
...
def check_current_conditions(lat, lon):
    ...
    try:
        ...
        rain = requests.get(w_url, timeout=10).json().get("current", {}).get("precipitation", 0)

        aqi_url = (
            f"https://air-quality-api.open-meteo.com/v1/air-quality?"
            f"latitude={lat}&longitude={lon}&current=pm2_5"
        )
        pm25 = requests.get(aqi_url, timeout=10).json().get("current", {}).get("pm2_5", 0)
